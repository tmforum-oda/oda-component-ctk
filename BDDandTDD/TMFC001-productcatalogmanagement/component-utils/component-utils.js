...
const componentUtils = {
  ...
  * @return   {Boolean}          True if the data matches the test data, false otherwise
  */
  validateReturnDataFromObject: async function (inResultData, inReturnData, inAPIURL) {
    let validatedSuccessfully = true

    // go through each item in test data and compare to return data
    for (const payload in inResultData.payloads) {
      for (const index in inResultData.payloads[payload]) {
        let body = inResultData.payloads[payload][index]
